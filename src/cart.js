...
/**
 * reason нужен, чтобы UI различал, чем подтвердить действие. Граница проходит
 * не между экранами, а по тому, показывает ли UI результат сам: там, где рядом
 * есть число (счётчик в карточке меню, строка в панели), хватает объявления
 * в aria-live; там, где числа нет (промо-блок «Напиток недели»), нужен тост.
...
/**
 * Добавление оттуда, где количество не видно, — сейчас это промо-блок
 * «Напиток недели». Показывает тост: подтвердить действие больше нечем.
 */
...
/**
 * «+» там, где число видно рядом: счётчик в карточке меню и строка в панели.
 * Тоста нет, только объявление. На отсутствующей позиции заводит её с qty 1 —
 * поэтому карточке меню отдельного «добавить» не нужно.
 */
export function incrementItem(id) {
  bump(id, 'increment');
}

/** «−» в панели и в карточке меню. На единице убирает позицию целиком. */
